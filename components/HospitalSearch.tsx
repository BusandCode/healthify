...
const HospitalSearch: React.FC = () => {
  ...
  const runSearch = (q: string, loc: string, filters: string[]): void => {
  ...
  const handleSearchSubmit = (): void => {
    runSearch(searchQuery, location, selectedFilters);
  };

  const handlePopularSearch = (term: string): void => {
    setSearchQuery(term);
    runSearch(term, location, selectedFilters);
  };
  ...
  return (
    <div className="w-full max-w-6xl mx-auto bg-white rounded-xl sm:rounded-2xl shadow-lg sm:shadow-xl border border-gray-100 p-4 sm:p-6 lg:p-8">
      ...
      {/* Filter Section — only shown once the user has picked at least one filter */}
      {selectedFilters.length > 0 && (
        <div className="space-y-3 sm:space-y-4">
          <h3 className="text-gray-700 font-medium text-sm sm:text-base">Filtering by</h3>
